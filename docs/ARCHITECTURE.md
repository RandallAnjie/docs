...
页面提醒保存在 `page_reminders`。创建者可以提醒自己或当前组织的正式成员，服务端返回的候选成员已经过页面查看权限过滤；创建、修改、取消和到期投递都会重新校验组织成员关系、页面 ACL、页面锁定与操作角色。提醒使用浏览器 IANA 时区、绝对到期时间和提前量保存，投递通过唯一事件键保证至多一条收件箱通知；收件箱请求还会补投当前用户的到期提醒，因此应用打开时不依赖客户端感知保存或刷新。平台定时事件处理器已具备全局后台投递入口，但 RandallFlare 当前没有为应用声明或创建 cron schedule 的控制面能力，正式环境尚不能保证用户完全离线时准点投递；详见 RF-11。
