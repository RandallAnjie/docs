...
`sites` 以普通页面为首页，`site_pages` 保存站点内稳定路径、显隐和导航顺序。发布与所有设置写入都要求首页的 `manage_access`；刷新页面清单通过有界递归读取首页子树，遇到 `restricted` 权限边界、数据库页或数据库模板页就停止向下公开。公开读取还会重新验证每一级祖先仍然可见，因此隐藏父页面会同时让所有后代返回 404。

未登录访问者没有组织成员身份。公开 API 只签发 5 分钟、角色固定为 `viewer` 的协作票据，公开同步块还要求容器页和来源页同时属于当前站点的可见页面集合。附件仍由私有 R2 提供，并通过当前公开站点及页面范围鉴权；访客不能取得评论、版本、权限或任何写 API。

站点设置包含路径、导航、系统/浅色/暗色主题、favicon、分享图、SEO 标题/描述、面包屑、站内搜索、Rdocs 标识、搜索引擎收录和 Google Analytics measurement ID。Worker 为每个公开页面输出 canonical、Open Graph 与 Twitter 元数据；只有管理员显式开启收录后才允许 robots 抓取并生成只含公开页面的 sitemap。

内置分析只保存按日聚合的浏览/搜索次数，以及 `site + 日期 + 浏览器会话` 的 SHA-256 摘要，不保存原始 IP 或用户标识。分析写入要求精确同源。当前数据库页面不会作为 Site 首页或穿过其子树公开；结构化数据库的站点只读视图属于下一批，而不是用空白文档假装支持。
...
通知投递和展示都重新计算当前页面 ACL。成员在投递后被撤权时，旧通知不会继续出现在列表或未读数字中；不同组织的未读数互不混合。页面内容变更使用 generation 和协作序列组成稳定事件键，D1 唯一索引避免 Durable Object 重试产生重复通知。收件箱响应同时保留扁平列表和按“页面 + 评论线程”生成的稳定分组；整组已读、未读、归档操作只接受当前用户通知 ID，评论分组可深链并聚焦到原线程。

页面提醒保存在 `page_reminders`。创建者可以提醒自己或当前组织的正式成员，服务端返回的候选成员已经过页面查看权限过滤；创建、修改、取消和到期投递都会重新校验组织成员关系、页面 ACL、页面锁定与操作角色。提醒使用浏览器 IANA 时区、绝对到期时间和提前量保存，投递通过唯一事件键保证至多一条收件箱通知；收件箱请求还会补投当前用户的到期提醒，因此应用打开时不依赖客户端感知保存或刷新。

提醒来源分为页面、正文内联节点和数据库日期属性。正文键入 `@remind`/`@提醒` 或使用斜杠菜单会创建带稳定 source ID 的协同原子节点；节点删除插件只在最后一个同源节点消失时调用幂等来源取消 API，远端协作删除的重复请求也不会产生副作用。数据库日期提醒以“数据库 + 行 + 属性”组成 source ID；清空日期、归档行或删除日期属性时，取消语句与数据库写入放在同一个 D1 batch，避免数据已变化但旧提醒仍触发。来源不能在修改提醒时被替换，活动来源使用部分唯一索引避免同一创建者重复创建。

评论线程分组可以直接在收件箱内回复，发送仍复用评论线程 API 并重新执行 `comment` 权限检查；成功后只把该线程组标记为已读。平台定时事件处理器已具备全局后台投递入口，但 RandallFlare 当前没有为应用声明或创建 cron schedule 的控制面能力，正式环境尚不能保证用户完全离线时准点投递；详见 RF-11。
