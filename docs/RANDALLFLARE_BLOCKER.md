...
详细根因、修复版本和验收证据保留在 [RandallFlare 平台问题修复清单](RANDALLFLARE_PLATFORM_ISSUES.md)。新发现的 RF-7 会让 D1 migration 账本写入空值，但已有可验证的精确迁移方案；RF-8 是低并发顺序产品 smoke 中间歇出现的 502，重跑可通过但仍需平台按 request ID 排查。
